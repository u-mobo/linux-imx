 ...
 */
static int id95apm_init(struct id95apm *id95apm, int irq,
			struct id95apm_platform_data *pdata)
{
	struct platform_device *pdev;
	int ret = -ENOMEM;
	u8 id;
	 ...
	 * GPIO subdevice
	 */
	pdev = id95apm_add_pdev(id95apm, &pdata->id95apm_gpio_init,
				"id95apm-gpio", 0);
	if (!pdev)
		goto err;
	 ...
	 */
	if (pdata && pdata->init) {
		ret = pdata->init(id95apm);
		if (ret != 0) {
			dev_err(id95apm->dev, "Platform init() failed: %d\n",
				ret);
			goto err;
		}
	}
	...
	if (irq) {
		...
		ret = request_irq(irq, id95apm_irq, flags, "id95apm", id95apm);
		if (ret != 0) {
 ...
 * cat /sys/class/i2c-dev/i2c-0/device/i2c-adapter:i2c-0/0-002a/dump
 *
 * Dump watchdog register:
 * cat /sys/class/i2c-dev/i2c-0/device/i2c-adapter:i2c-0/0-002a/dump_wdt
 *
 * Dump ldo/dcdc register:
 * cat /sys/class/i2c-dev/i2c-0/device/i2c-adapter:i2c-0/0-002a/dump_ldo
 *
 * Dump charger register:
 * cat /sys/class/i2c-dev/i2c-0/device/i2c-adapter:i2c-0/0-002a/dump_charge
 *
 * Dump rtc register:
 * cat /sys/class/i2c-dev/i2c-0/device/i2c-adapter:i2c-0/0-002a/dump_rtc
 */
...
{
	struct id95apm_platform_data *pdata = i2c->dev.platform_data;
	struct id95apm *id95apm;
	int ret;

	id95apm = kzalloc(sizeof(struct id95apm), GFP_KERNEL);
	if (id95apm == NULL) {
		ret = -ENOMEM;
		goto err;
	}
	...
	ret = id95apm_init(id95apm, i2c->irq, pdata);
	if (ret)
		goto struct_err;
...
static int __init id95apm_module_init(void)
{
	int ret;

	ret = i2c_add_driver(&id95apm_i2c_driver);
	if (ret != 0)
...
subsys_initcall(id95apm_module_init);

static void __exit id95apm_module_exit(void)
{
	i2c_del_driver(&id95apm_i2c_driver);
}
module_exit(id95apm_module_exit);
