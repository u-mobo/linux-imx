...
static int __devinit id95apm_hwmon_probe(struct platform_device *pdev)
{
	 ...
	 */
	id95apm_clrset_bits16(id95apm, ID95APM_PCON_GPIO_OFF, 0x0000, 0xffc0);
...
static int __devexit id95apm_hwmon_remove(struct platform_device *pdev)
{
	struct id95apm *id95apm = dev_get_drvdata(pdev->dev.parent);
...
static struct platform_driver id95apm_hwmon_driver = {
	...
	.remove = __devexit_p(id95apm_hwmon_remove),
	.driver = {
		.name = "id95apm-hwmon",
		.owner = THIS_MODULE,
	},
