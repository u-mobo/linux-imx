			     ...
			     struct l3g4200d_output *data)
{
	...
	gyro_out[0] = (AUTO_INCREMENT | OUT_TEMP);

	err = l3g4200d_i2c_read(gyro, gyro_out, 8);

	if (err < 0)
		return err;

	hw_d[0] = (s16) (((gyro_out[3]) << 8) | gyro_out[2]);
	hw_d[1] = (s16) (((gyro_out[5]) << 8) | gyro_out[4]);
	hw_d[2] = (s16) (((gyro_out[7]) << 8) | gyro_out[6]);

	data->temp = gyro->pdata->temp_calibration - gyro_out[0];
				     ...
				     char *buf)
{
	...
	struct l3g4200d_data *gyro = dev_get_drvdata(dev);
	mutex_lock(&gyro->lock);
	val = gyro->pdata->temp_calibration;
	mutex_unlock(&gyro->lock);
	return sprintf(buf, "%d\n", val);
}
				     ...
				     const char *buf, size_t size)
{
	...
	if ((tempcal <= 0) || (tempcal > 64))
		return -EINVAL;
	mutex_lock(&gyro->lock);
	gyro->pdata->temp_calibration = tempcal;
	mutex_unlock(&gyro->lock);
	return size;
}
...
static void l3g4200d_input_poll_func(struct input_polled_dev *dev)
{
	...
	if (err < 0)
		dev_err(&gyro->client->dev, "l3g4200d_get_data failed\n");
	else
