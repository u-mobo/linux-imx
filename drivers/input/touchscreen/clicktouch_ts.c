...
static int clicktouch_ts_open(struct input_dev *dev)
{
	...
	/* power on the device */
	if (ts->platform_data && ts->platform_data->power_on) {
		rc = ts->platform_data->power_on(1);
		if (rc) {
	...
	clicktouch_ts_write_reg_u8(ts->client, 0x00, 0x01); /* issue reset */

	if (ts->touch_irq) {
		rc = request_irq(ts->touch_irq, clicktouch_ts_irq,
...
static void clicktouch_config_touch_irq(struct clicktouch_ts *ts)
{
	struct gpio_keys_button *gpio_irq = &(ts->platform_data->gpio_irq);
	int rc;

	rc = gpio_request(gpio_irq->gpio, "clicktouch_gpio_irq");
	if (rc) {
		dev_err(&ts->client->dev, "unable to request gpio %d\n", gpio_irq->gpio);
		return;
	}

	rc = gpio_direction_input(gpio_irq->gpio);
	if (rc) {
		dev_err(&ts->client->dev, "unable to set direction for gpio %d\n", gpio_irq->gpio);
		return;
	}

	ts->touch_irq = gpio_to_irq(gpio_irq->gpio);
}
...
static int clicktouch_ts_init(struct i2c_client *client, struct clicktouch_ts *ts)
{
	...
	if (ts->platform_data) {
		...
		ts->invert_x = ts->platform_data->flags & CLICKTOUCH_TS_INVERT_X;
		ts->invert_y = ts->platform_data->flags & CLICKTOUCH_TS_INVERT_Y;
		ts->swap_xy = ts->platform_data->flags & CLICKTOUCH_TS_SWAP_XY;
	...
	if (ts->swap_xy) {
		input_set_abs_params(input_device, ABS_MT_POSITION_X, 0, ts->max_y, 0, 0);
		input_set_abs_params(input_device, ABS_MT_POSITION_Y, 0, ts->max_x, 0, 0);
	 } else {
	...
	if (ts->swap_xy) {
		input_set_abs_params(input_device, ABS_X, 0, ts->max_y, 0, 0);
		input_set_abs_params(input_device, ABS_Y, 0, ts->max_x, 0, 0);
	} else {
...
static int clicktouch_ts_suspend(struct device *dev)
{
	...
	if (ts->touch_irq)
		disable_irq_nosync(ts->touch_irq);
	...
	if (ts->platform_data && ts->platform_data->power_on) {
		rc = ts->platform_data->power_on(0);
		if (rc) {
...
static int __devinit clicktouch_ts_probe(struct i2c_client *client,
...
static int __devexit clicktouch_ts_remove(struct i2c_client *client)
{
...
static struct i2c_driver clicktouch_ts_driver = {
	...
	.remove		= __devexit_p(clicktouch_ts_remove),
