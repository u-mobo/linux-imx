...
static int __devinit id95apm_wdt_probe(struct platform_device *pdev)
{
...
static int __devexit id95apm_wdt_remove(struct platform_device *pdev)
{
	struct id95apm *id95apm = platform_get_drvdata(pdev);
...
static struct platform_driver id95apm_wdt_driver = {
	...
	.remove = __devexit_p(id95apm_wdt_remove),
	.driver = {
		.name = "id95apm-wdt",
	},
};
