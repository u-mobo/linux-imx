...
#define ABS_TEMPERATURE          	ABS_MISC
...
static int mpl3115_i2c_read(struct i2c_client *client, u8 reg, u8 * values,
			    u8 length)
{
	return i2c_smbus_read_i2c_block_data(client, reg, length, values);
}

static int mpl3115_i2c_write(struct i2c_client *client, u8 reg,
			     const u8 * values, u8 length)
{
...
static DEVICE_ATTR(enable, S_IWUSR | S_IRUGO, mpl3115_enable_show,
		   mpl3115_enable_store);
...
static int mpl3115_read_data(struct i2c_client *client, int *pres, short *temp)
{
	...
	*pres =
	    (DATA_SHIFT_BIT(tmp[0], 24) | DATA_SHIFT_BIT(tmp[1], 16) |
	     DATA_SHIFT_BIT(tmp[2], 8)) >> 12;
...
static void report_abs(struct mpl3115_data *pdata)
{
	...
	input_report_abs(idev, ABS_TEMPERATURE, temperature);
	input_sync(idev);
 out:
	mutex_unlock(&pdata->data_lock);
}
				   ...
				   const struct i2c_device_id *id)
{
	...
	input_set_abs_params(idev, ABS_TEMPERATURE, -0x7FFFFFFF, 0x7FFFFFFF, 0,
			     0);
