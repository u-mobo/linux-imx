...
static int __devinit id95apm_codec_platform_probe(struct platform_device *pdev)
{
	return snd_soc_register_codec(&pdev->dev, &soc_codec_dev_id95apm,
			&id95apm_codec_dai, 1);
}

static int __devexit id95apm_codec_platform_remove(struct platform_device *pdev)
{
...
static struct platform_driver id95apm_codec_platform_driver = {
	...
	.remove	= __devexit_p(id95apm_codec_platform_remove),
};
